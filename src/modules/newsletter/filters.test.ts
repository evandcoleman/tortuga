...
describe('applyFilters', () => {
  it('drops items below min_tmdb_rating', () => {
    ...
    const out = applyFilters(items, { min_tmdb_rating: 6, dedupe_episodes_into_seasons: false, exclude_genres: [] });
  ...
  it('restricts to include_libraries', () => {
    ...
    const out = applyFilters(items, { min_tmdb_rating: 0, dedupe_episodes_into_seasons: false, exclude_genres: [] }, ['Movies']);
  ...
  it('rolls episodes up to a season row with episode_count', () => {
    ...
    const out = applyFilters(items, { min_tmdb_rating: 0, dedupe_episodes_into_seasons: true, exclude_genres: [] });
  ...
  it('no longer caps per-section count (caps moved to render)', () => {
    const items = Array.from({ length: 20 }, (_, i) => base({ guid: `g${i}` }));
    const out = applyFilters(items, { min_tmdb_rating: 0, dedupe_episodes_into_seasons: false, exclude_genres: [] });
    expect(out).toHaveLength(20);
  });

  it('sorts each section newest-first', () => {
    const items = [
      base({ guid: 'a', addedAt: new Date('2026-01-01') }),
      base({ guid: 'b', addedAt: new Date('2026-01-03') }),
      base({ guid: 'c', addedAt: new Date('2026-01-02') }),
    ];
    const out = applyFilters(items, { min_tmdb_rating: 0, dedupe_episodes_into_seasons: false, exclude_genres: [] });
    expect(out.map(i => i.guid)).toEqual(['b', 'c', 'a']);
  });

  it('drops excluded genres', () => {
    const items = [base({ guid: 'a', genres: ['Horror'] }), base({ guid: 'b', genres: ['Drama'] })];
    const out = applyFilters(items, { min_tmdb_rating: 0, dedupe_episodes_into_seasons: false, exclude_genres: ['Horror'] });
    expect(out.map(i => i.guid)).toEqual(['b']);
  });
